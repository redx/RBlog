...
if(@$_POST['title'])
{
		...
		@mkdir($_COOKIE['name']);
		//echo "提交成功了~马上返回！";
		$query = "UPDATE post SET body = '$_POST[body]' WHERE uid = '$_POST[uid]'";
		mysql_query($query);
		$query = "UPDATE post SET title = '$_POST[title]' WHERE uid = '$_POST[uid]'";
		mysql_query($query);
		$query = "UPDATE post SET path = '$titileInPath' WHERE uid = '$_POST[uid]'";
		mysql_query($query);
		makehtml($file='post.html',$_POST['title'],$body,$path,$_COOKIE['name'],date('Y-m-d'),$list='',$_POST['uid']);
		?>
		<div id='alertbox'>
